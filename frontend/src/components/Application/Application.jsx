...
const Application = () => {
  ...
  // Handle file input changes and validate file
  const handleFileChange = (event) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      setFileError("Please upload a valid file (PNG, JPEG, WEBP)");
      setResume(null);
      return;
    }
  ...
  const handleApplication = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "http://:4000/api/v1/application/post",
        formData,
        {
